...
didx  = pd.todatetime(prc['Date'], format='%Y-%m-%d')
#print(didx)
...
#in class activity 1
x = dt.datetime(year= 2023, month=12, day= 9, 10, 23)
s = x.strftime(%Y)

#inclass activity 2
CSVLOC = os.path.join(cfg.DATADIR, 'tsla_prc.csv')
Date = dt.datetime(year=2019,month=3, day= 15)
tradeday= pd.to_datetime(prc['Date'], format'%Y')
Adjclose= Date.set_index('Adj Close')
